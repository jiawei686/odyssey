...
| FR-26 | One independent volumetric medical-assistant avatar appears beside the library at app launch. Gaze-plus-pinch toggles the conversation panel while the avatar remains visible, and a seven-second idle motion stays within 14 degrees of its forward pose. The toolbar may hide or restore both scenes without granting control over anatomy or tracking. | Scene/source invariant, clean simulator launch/toggle/motion review, and physical gaze/pinch, comfort, and Reduce Motion review. |
...
| FR-31 | Voice input starts only after an explicit microphone action, with on-device partial transcription visible in real time, an explicit Text alternative, streamed answer text for both providers, and system speech playback after completion. Captured audio is not saved or sent as audio to either model provider. | Source/permission and push-to-talk contract, simulator streaming review, and physical microphone, locale, silence, playback, and interruption tests. |
...
- The direct third-party LLM connection is POC-only. Distribution requires a team backend proxy plus documented provider retention, processing location, access control, rate limits, and deletion policy.
- Foundation Models integration requires visionOS 26 at runtime and must expose device eligibility, Apple Intelligence disabled, model-not-ready, and unsupported-locale states without silently selecting the cloud provider.
...
| Medical assistant avatar and voice | Launch-presented avatar, pinch-toggled panel, bounded idle motion, semantic anatomy context, explicit push-to-talk, live transcript, streaming answers, speech playback, bounded memory, Keychain configuration, local safety handling, and citation allow-listing work without renderer authority. | Contract and Vision simulator build pass; updated clean launch/toggle/motion visual review plus physical voice/gaze interaction, provider availability, and named clinical source review pending. |
