...
| FR-30 | Voice is the default assistant input with on-device partial transcription visible in real time, an explicit Text alternative, streamed answer text for both providers, and system speech playback after completion. Captured audio is not saved or sent as audio to either model provider. | Source/permission contract, simulator streaming review, and physical microphone, locale, silence, playback, and interruption tests. |
...
| Medical assistant | Launch-presented avatar, pinch-toggled panel, bounded idle motion, semantic anatomy context, bounded memory, Keychain configuration, local safety handling, live transcript, streaming answers, speech playback, and citation allow-listing work without renderer authority. | Contract and Vision simulator build pass; updated clean launch/toggle/motion visual review plus physical voice/gaze interaction, provider availability, and named clinical source review pending. |
